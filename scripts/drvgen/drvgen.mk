...
$(DRVGEN_FILE_LIST): $(DRVGEN_TOOL) $(DWS_FILE) $(DRVGEN_FIG) $(PROJ_DTS_FILES)
	for i in $(PROJ_DTS_FILES); do \
		...
		if [ -f $$dws_path ] ; then \
			mkdir -p $$prj_path ;\
			PYTHONPATH=$(srctree)/tools /usr/bin/python3 -m dct.DrvGen $$dws_path $(abspath $(DRVGEN_OUT))/$$base_prj $(abspath $(DRVGEN_OUT))/$$base_prj cust_dtsi; \
		fi \
	done

dtbo_check: $(MAIN_DTB_NAMES) $(PROJ_DTB_NAMES)
	for i in $(PROJ_DTB_FILES); do \
		/usr/bin/fdtoverlay -i $(MAIN_DTB_FILES) -o $$i.merge $$i;\
	done
